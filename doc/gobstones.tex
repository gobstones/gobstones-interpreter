...
\]
Los identificadores \tok{LOWERID} comienzan por un caracter en min\'usculas y
los identificadores \tok{UPPERID} comienzan por un caracter en may\'usculas.
...
\produccion{\s{exprConstructor}}{
  \tok{UPPERID} (\tok{LPAREN} \s{fieldBindingSeq} \tok{RPAREN})?
}

\produccion{\s{exprConstructorUpdate}}{
  \tok{UPPERID} \tok{LPAREN} \s{expression} \tok{PIPE} \s{fieldBindingSeq} \tok{RPAREN}
}

\produccion{\s{fieldBinding}}{
  \tok{LOWERID} \tok{GETS} \s{expression}
}

\produccion{\s{fieldBindingSeq}}{
  \EMPTY
\ALT
...
\ALT
  \ast{ExprConstructor}(constructorName : \type{ID}, fieldBindings : [\type{FieldBinding}])
\ALT
  \ast{ExprConstructorUpdate}(constructorName : \type{ID}, original : \type{Expression}, fieldBindings : [\type{FieldBinding}])
\ALT
...
\datadecl{\type{FieldBinding}}{
  \ast{FieldBinding}(fieldName : \type{ID}, value : \type{Expression})
}
