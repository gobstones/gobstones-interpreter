...
\symbolentry{\texttt{"(\textbackslash{t}|\textbackslash{n}|\textbackslash{r}|\textbackslash\textbackslash|\textbackslash"|[\^{}\textbackslash"])*"}}{\tok{STRING}}{\nb{Constante {\em string}.}}
\end{tabularx}
\medskip
...
Esta gram\'atica es liberal en algunos sentidos. En particular:
\begin{itemize}
...
({\em lint}).
\item El \texttt{return} recibe una lista de valores a devolver.
\end{itemize}
...
\produccion{\s{program}}{\s{declaration}*}

\produccion{\s{declaration}}{
\s{programDeclaration}
\ALT
\s{interactiveProgramDeclaration}
\ALT
\s{procedureDeclaration}
\ALT
...
\produccion{\s{programDeclaration}}{
  \tok{PROGRAM} \s{stmtBlock}
}

\produccion{\s{interactiveProgramDeclaration}}{
  \TODO{TODO}
}

\produccion{\s{procedureDeclaration}}{
  \tok{PROCEDURE} \tok{LPAREN} \s{parameterList} \tok{RPAREN} \s{stmtBlock}
}

\produccion{\s{functionDeclaration}}{
  \tok{FUNCTION} \tok{LPAREN} \s{parameterList} \tok{RPAREN} \s{stmtBlock}
}

\produccion{\s{typeDeclaration}}{
  \TODO{TODO}
}

\produccion{\s{parameterList}}{
  \EMPTY
\ALT
  \s{nonEmptyParameterList}
}

\produccion{\s{nonEmptyParameterList}}{
  \tok{LOWERID} (\tok{COMMA} \s{nonEmptyParameterList})?
}
...
\produccion{\s{expressionList}}{
  \EMPTY
\ALT
  \s{nonEmptyExpressionList}
}

\produccion{\s{nonEmptyExpressionList}}{
  \s{expression} (\tok{COMMA} \s{nonEmptyExpressionList})?
}
