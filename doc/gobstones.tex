...
Esta gram\'atica es liberal en algunos sentidos. En particular, el \texttt{return}
...
pero las convenciones de asociatividad y precedencia de operadores no
se reflejan en las producciones, sino por fuera.
\bigskip
...
\produccion{\s{procedureDeclaration}}{
  \tok{PROCEDURE} \tok{LPAREN} \s{parameterList} \tok{RPAREN} \s{block}
}

\produccion{\s{functionDeclaration}}{
  \tok{FUNCTION} \tok{LPAREN} \s{parameterList} \tok{RPAREN} \s{block}
}
