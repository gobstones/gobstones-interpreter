...
\produccion{\s{exprFunctionCall}}{
  \tok{LOWERID} \tok{LPAREN} \s{expressionList} \tok{RPAREN}
}
...
\produccion{\s{exprConstructor}}{
  \tok{UPPERID} (\tok{LPAREN} \s{fieldValueList} \tok{RPAREN})?
}

\produccion{\s{exprConstructorUpdate}}{
  \tok{UPPERID} \tok{LPAREN} \s{expression} \tok{PIPE} \s{fieldValueList} \tok{RPAREN}
}
...
\produccion{\s{fieldValueList}}{
  \EMPTY
\ALT
  \s{\nonEmpty{fieldValueList}}
}
